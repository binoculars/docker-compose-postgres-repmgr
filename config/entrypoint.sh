...
if [ ! -s "$PGDATA/PG_VERSION" ]; then
    ...
    sed \
        ...
        -e "s|^conninfo='host=127.0.0.1 dbname=repmgr user=repmgr'|conninfo='host=$(hostname) dbname=repmgr user=${REPMGR_USER} password=${REPMGR_PASSWORD} application_name=node${ordinal_plus1}'|" \
        /etc/_repmgr.conf > /etc/repmgr.conf

    # Master
    if [[ $ordinal -eq 0 ]]; then
        gosu postgres repmgr --verbose master register
    # Standby
    else
        if [[ ! -f "${PGDATA}/recovery.conf" ]]; then
            ...
            gosu postgres repmgr \
                --dbname="host=${MASTER_HOST} dbname=repmgr user=${REPMGR_USER} password=${REPMGR_PASSWORD} application_name=node1" \
                standby clone
            gosu postgres pg_ctl -D "$PGDATA" -w start
        fi
